...
public class AccountingNeeds {
	 ...
	 * each function called from within; these should be distinct, even if they are
	 * multiple calls to the same function. The generated file format provides
	 * for a means to distinguish these by line number.
	 *
	 * Note that because of Java's capability to throw exceptions, we have to
	 * wrap the function body, and all function calls, with try-finally in order
	 * to be sure we capture the end time.
	 *
	 ...
	 * callgrind4j_methodName_lineNumber_methodName
	 * Note that this won't distinguish between multiple calls on the same line,
	 ...
	 * conveniently solves the problem of what to do in the face of multiple
	 * class loaders, such as those used by Drill.
	 */
	...
	public void instrumentedMethod(final SomeClass someClass) {
		...
		try {
			...
			try {
				...
				instrumentedMethod_15_a.addAndGet(System.nanoTime()
						- callStartTime);
			}
			...
			try {
				...
				instrumentedMethod_19_b.addAndGet(System.nanoTime()
						- callStartTime);
			}
		} finally {
